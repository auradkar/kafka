...
class QuotasTest extends KafkaServerTestHarness {
  ...
  def testProducerConsumerOverrideUnthrottled() {
    ...
    assertEquals("Should not have been throttled", Double.NaN, allMetrics(producerMetricName).value(), EPS)
    ...
    assertEquals("Should not have been throttled", Double.NaN, allMetrics(consumerMetricName).value(), EPS)
  }
